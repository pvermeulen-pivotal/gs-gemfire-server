...
import io.pivotal.gemfire.server.statistics.BeaconRequestStatistics;

public class BeaconRequestAsyncListener implements AsyncEventListener {
	private final String ERROR_REGION = "GreenplumErrorQueue";
	private final int ERROR_LOG_LIMIT = 25;
	private final Logger log = LoggerFactory.getLogger(BeaconRequestAsyncListener.class);
	...
	private BeaconRequestStatistics beaconReqStat;

	public BeaconRequestAsyncListener() {
		ctx = new ClassPathXmlApplicationContext("classpath*:gemfire-beacon-request-context.xml");
		...
		beaconReqStat = new BeaconRequestStatistics();
		initialize();
		delay = (long) CacheFactory.getAnyInstance().getAsyncEventQueue("beacon-request").getBatchTimeInterval();
	}

	private void initialize() {
		String mapping = null;
		StringWriter writer = new StringWriter();
		InputStream stream = BeaconRequestAsyncListener.class.getResourceAsStream("/beacon-request-mapping.txt");
		try {
			...
			log.error("BeaconRequest Async Listener failed during initialization- exception: " + e.getMessage());
		}
	}
	...
	public boolean processEvents(List<AsyncEvent> events) {
		...
		if (initialized) {
			...
			beaconReqStat.setCreateGPBatchTime(System.currentTimeMillis() - startTm);
			beaconReqStat.setNumberEntries(events.size());
			statEntryTimer(delay);
			if (strBuilder.length() > 0) {
				startTm = System.currentTimeMillis();
				boolean ret = processExternalTable(strBuilder);
				beaconReqStat.setGreenplumUpdateTime(System.currentTimeMillis() - startTm);
				return ret;
			} else {
				return true;
			}
	...
	private boolean processEvent(AsyncEvent<?, ?> event, StringBuilder strBuilder, Date date) {
		...
		if (errorMessage != null) {
			beaconReqStat.updateGreenplumDatabaseErrors(1);
	...
	private void statEntryTimer(long delay) {
		task = new TimerTask() {
			public void run() {
				beaconReqStat.setNumberEntries(0);
			}
		};
